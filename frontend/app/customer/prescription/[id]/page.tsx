...
export default function PrescriptionDetailPage() {
  ...
  const handleDownload = async () => {
    try {
      const token = localStorage.getItem("token")
      const response = await fetch(api(`/api/appointment/prescriptions/${id}/download/`), {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50 flex items-center justify-center p-4">
        <Card className="max-w-md w-full shadow-lg">
          <CardContent className="p-6 text-center">
            ...
            <p className="text-gray-600 mb-4 text-sm">The prescription you're looking for doesn't exist.</p>
            <Button onClick={() => router.back()} className="bg-blue-600 hover:bg-blue-700">
              Go Back
            </Button>
          </CardContent>
          ...
          <CardContent className="p-8">
            ...
            <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-8 pb-8 border-b-2 border-gray-100">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <Badge className="bg-green-100 text-green-800 border-0">Valid & Active</Badge>
              ...
              <div className="text-right">
                ...
                <p className="text-lg font-mono font-semibold text-blue-600">#{prescription.id}</p>
              </div>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-8 mb-8">
                ...
                <div className="space-y-4">
                  <div className="flex gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-gray-900 text-lg">
                        {prescription.doctor_name || prescription.doctor?.user?.name || "Dr. Name"}
                      </p>
                      <p className="text-sm text-gray-600 mt-1">
                        {prescription.doctor_specialty || "General Practitioner"}
                      </p>
                      {prescription.doctor_license && (
                        <p className="text-xs text-gray-500 mt-1">License: {prescription.doctor_license}</p>
                      )}
                    </div>
                  </div>
                  {prescription.clinic_name && (
                    <div className="flex gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium text-gray-900">{prescription.clinic_name}</p>
                        {prescription.clinic_address && (
                          <p className="text-xs text-gray-600">{prescription.clinic_address}</p>
                        )}
                      </div>
                    </div>
                  )}
                  {prescription.doctor_contact && (
                    <div className="flex gap-3">
                      ...
                      <p className="text-sm text-gray-600">{prescription.doctor_contact}</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-4">
                  <div>
                    <p className="font-semibold text-gray-900 text-lg">
                      {prescription.patient_name || prescription.patient?.name || "Patient Name"}
                    </p>
                    {prescription.patient_age && (
                      <p className="text-sm text-gray-600 mt-1">Age: {prescription.patient_age} years</p>
                    )}
                    {prescription.patient_gender && (
                      <p className="text-sm text-gray-600">Gender: {prescription.patient_gender}</p>
                    )}
                  </div>
                  ...
                  {prescription.patient_phone && (
                    <div className="flex gap-3">
                      <Phone className="w-5 h-5 text-blue-600 flex-shrink-0 mt-0.5" />
                      <p className="text-sm text-gray-600">{prescription.patient_phone}</p>
                    </div>
                  )}
                  <div className="pt-2 border-t border-gray-100">
                    <p className="text-xs text-gray-500">Appointment ID</p>
                    <p className="font-mono text-sm font-semibold text-gray-900">{prescription.appointment_id}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-4 mb-8 p-4 bg-blue-50 rounded-lg border border-blue-100">
              ...
              <div>
                <p className="text-xs font-semibold text-gray-700 uppercase">Appointment Time</p>
                <div className="flex items-center gap-2 mt-1">
                  <Clock className="w-4 h-4 text-blue-600" />
                  <p className="text-sm font-medium text-gray-900">{prescription.appointment_time || "N/A"}</p>
                </div>
              </div>
              <div>
                <p className="text-xs font-semibold text-gray-700 uppercase">Prescription Date</p>
                <div className="flex items-center gap-2 mt-1">
                  <Calendar className="w-4 h-4 text-blue-600" />
                  <p className="text-sm font-medium text-gray-900">
                    {prescriptionDate.toLocaleDateString("en-US", {
                            ...
                            <div className="grid sm:grid-cols-2 gap-3 text-sm">
                              ...
                              {med.refills && (
                                <div>
                                  ...
                                  <p className="text-gray-600">{med.refills}</p>
                                </div>
                              )}
                            </div>
                          )}
            ...
            {prescription.follow_up_date && (
              <div className="mb-8 p-4 bg-green-50 border-l-4 border-green-500 rounded">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h3 className="text-sm font-semibold text-gray-700 uppercase mb-1">Follow-up Appointment</h3>
                    ...
                    <p className="text-sm text-green-700 mt-1">
                      Please schedule your follow-up appointment as recommended by your physician.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="border-t-2 border-gray-200 pt-6 mt-8">
              <div className="grid md:grid-cols-3 gap-4 mb-6">
                <div className="text-center">
                  <p className="text-xs font-semibold text-gray-600 uppercase">Validity Status</p>
                  <Badge className="mt-2 bg-green-100 text-green-800 border-0 mx-auto">Valid & Active</Badge>
                </div>
                <div className="text-center">
                  <p className="text-xs font-semibold text-gray-600 uppercase">Issued</p>
                  <p className="text-sm font-medium text-gray-900 mt-2">{prescriptionDate.toLocaleString()}</p>
                </div>
                <div className="text-center">
                  <p className="text-xs font-semibold text-gray-600 uppercase">Prescription #</p>
                  <p className="text-sm font-mono font-semibold text-gray-900 mt-2">{prescription.id}</p>
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-gray-200 text-center space-y-1">
                ...
                <p className="text-xs text-gray-500">
                  For inquiries, please contact your healthcare provider or the issuing clinic.
                </p>
              </div>
            </div>
          </CardContent>
